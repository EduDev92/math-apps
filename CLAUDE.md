...
3. **Never build a long single-line "a=b=c=d=e" chained equation - and never rely on horizontal scroll to hide one.** KaTeX's own `.katex-display>.katex` is `text-align:center; white-space:nowrap` - a chain wider than its container gets silently cropped on **both edges** on a phone, not scrolled (confirmed - overriding `text-align` does not fix this; CSS cannot fix it). Horizontal touch-scroll is not a substitute fix either: in RTL layout the scroll fallback silently never triggers unless `.katex-display` also has `direction:ltr` (see `bug_katex_rtl_scrollwidth` in memory). Use `eqRows([...])` from `math-format.js` to stack it as separate rows instead.
   - **Default target: 2 rows.** Row 1 = formula definition + parameter substitution (e.g. `m=\dfrac{y_2-y_1}{x_2-x_1}=\dfrac{2-(-2)}{0-(-3)}`); Row 2 = the final simplified/reduced result (e.g. `=\dfrac{4}{3}`). Don't pad with a redundant duplicate row (never repeat the same simplified result on two lines).
   - Only go past 2 rows when a row still overflows at 360px (rule 8) after simplification - e.g. a `\sqrt{}` combining two substitutions. Split that row's content further at the call site rather than accepting the overflow (`\\` cannot be used inside an open `\sqrt{}`/`\dfrac{}` argument - the split has to happen in how you build the strings, not inside `eqRows`). One substitution step per extra row.
